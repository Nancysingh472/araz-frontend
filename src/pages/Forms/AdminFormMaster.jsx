...
const AdminFormMaster = () => {
  ...
  const loadForms = useCallback(async () => {
    ...
    try {
      ...
      const result = await fetchForms(page, limit, search, statusFilter);
  ...
  const handleSubmit = async (data) => {
    try {
      if (data.id) {
        ...
        await editForm(id, dataWithoutId);
      } else {
        const addedItem = await createForm(data);
        navigate(`/admin/FormEdit?formId=${addedItem?.data?.id}`);
      }
    } catch (error) {
                            ...
                            <tr key={item.id}>
                              ...
                              <td>
                                <div className="action-btn-list">
                                  <Link
                                    to={`/admin/FormEdit?formId=${item.id}`}
                                  >
