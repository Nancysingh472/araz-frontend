...
const RazaComments = ({ requestId }) => {
  ...
  const loadRequestComments = useCallback(async () => {
    try {
      const result = await fetchRequestComments(requestId);
  ...
  const handleSubmit = async () => {
    try {
      const params = {
        razaRequestId: requestId,
        comment: addedComment,
      };
      await createRequestComments(params);
      setAddedComments('');
    } catch (error) {
  ...
  return (
    ...
    >
      <div className="offcanvas-header">
        <h5 id="offcanvasRightLabel">All Comments</h5>
